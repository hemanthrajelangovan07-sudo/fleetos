...
import {
  ...
  Activity, TrendingUp, Bell, Zap, Thermometer, Gauge, Fuel, MapPin,
  Download, RefreshCw, ChevronRight, Radio, AlertTriangle, Shield, Server
} from "lucide-react";
...
const rl=(l)=>({LOW:T.green,MEDIUM:T.amber,HIGH:T.red,UNKNOWN:T.muted}[l]||T.muted);
const rb=(l)=>({LOW:T.greenBg,MEDIUM:T.amberBg,HIGH:T.redBg,UNKNOWN:'rgba(148,163,184,0.08)'}[l]||'transparent');
...
// ─── TELEMETRY SIMULATOR (mirrors simulator.py) ───────────────────────
const SIM_VEHICLES=[
...
function initSimState(){
  ...
  SIM_VEHICLES.forEach(v=>{
    s[v.id]={speed:rnd(30,80),temp:rnd(70,85),
      battery:v.type==='EV'?rnd(50,90):null,
      fuel:v.type!=='EV'?rnd(20,60):null,
      rpm:rnd(1500,3000),lat:12.9716+rnd(-0.05,0.05),lon:80.2709+rnd(-0.05,0.05),
      anomalyCountdown:Math.round(rnd(20,60)),activeAnomaly:null,tick:0};
  });
  return s;
}
...
function tickSimState(prev){
  ...
  SIM_VEHICLES.forEach(v=>{
    ...
    s.lat+=rnd(-0.0005,0.0005); s.lon+=rnd(-0.0005,0.0005);
    if(v.type==='EV'){s.battery=clamp(s.battery-s.speed*0.0003+rnd(-0.1,0.2),5,100);}
    else{s.fuel=clamp(s.fuel-rnd(0.01,0.05),0,60);}
    s.anomalyCountdown--;
    s.activeAnomaly=null;
    if(s.anomalyCountdown<=0){
      const pool=v.type==='EV'?['overheat','overspeed','low_battery']:['overheat','overspeed'];
      const a=pool[Math.floor(Math.random()*pool.length)];
      if(a==='overheat'){s.temp=rnd(102,110);s.activeAnomaly='overheat';}
      else if(a==='overspeed'){s.speed=rnd(135,155);s.activeAnomaly='overspeed';}
      else if(a==='low_battery'){s.battery=rnd(5,12);s.activeAnomaly='low_battery';}
...
function toReading(vid,s,ts){
  return{vehicle_id:vid,timestamp:ts||new Date().toISOString(),
    speed:+s.speed.toFixed(1),temperature:+s.temp.toFixed(1),
    ...
    latitude:+s.lat.toFixed(6),longitude:+s.lon.toFixed(6),activeAnomaly:s.activeAnomaly};
}

function buildAlerts(readings,vid){
  const r=readings[0];
  if(!r)return{risk_level:'LOW',alerts:[],summary:'No data.'};
  const alerts=[];
  if(r.speed>130)alerts.push({metric:'speed',issue:`Overspeed: ${r.speed} km/h`,recommendation:'Reduce speed immediately.'});
  if(r.temperature>100)alerts.push({metric:'temperature',issue:`Overheating: ${r.temperature}°C`,recommendation:'Pull over. Check coolant.'});
  if(r.battery_pct!=null&&r.battery_pct<15)alerts.push({metric:'battery_pct',issue:`Critical battery: ${r.battery_pct}%`,recommendation:'Charge immediately.'});
  if(r.engine_rpm>6000)alerts.push({metric:'engine_rpm',issue:`Over-revving: ${r.engine_rpm} RPM`,recommendation:'Reduce throttle and upshift.'});
  if(readings.length>3&&(readings[0].temperature-readings[3].temperature)>8)alerts.push({metric:'temperature',issue:'Rapid temperature spike',recommendation:'Monitor cooling system.'});
  const risk=alerts.length===0?'LOW':alerts.some(a=>a.metric==='temperature'||a.metric==='speed')&&r.temperature>104?'HIGH':'MEDIUM';
  return{risk_level:risk,alerts,summary:alerts.length===0?'All metrics within safe operating ranges.':
    `${alerts.length} issue(s) detected for ${vid}.`};
}

function computeStats(history){
  if(!history.length)return null;
  const spd=history.map(r=>r.speed).filter(Boolean);
  const tmp=history.map(r=>r.temperature).filter(Boolean);
  const bat=history.map(r=>r.battery_pct).filter(Boolean);
  const rpm=history.map(r=>r.engine_rpm).filter(Boolean);
  const avg=a=>a.length?+(a.reduce((s,v)=>s+v,0)/a.length).toFixed(1):0;
  return{total_readings:history.length,
...
const Badge=({label,color,bg})=>(
  <span style={{display:'inline-flex',alignItems:'center',padding:'3px 10px',borderRadius:20,
    fontSize:12,fontWeight:500,color,background:bg,border:`1px solid ${color}33`,whiteSpace:'nowrap'}}>{label}</span>
);
const Modal=({title,onClose,children,wide})=>(
  <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.75)',display:'flex',alignItems:'center',
    justifyContent:'center',zIndex:1000,backdropFilter:'blur(4px)',padding:16}}>
    <div style={{background:T.card,border:`1px solid ${T.borderHi}`,borderRadius:16,padding:32,
      width:'100%',maxWidth:wide?800:520,maxHeight:'92vh',overflowY:'auto',boxShadow:'0 30px 60px rgba(0,0,0,0.6)'}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:24}}>
        <h2 style={{color:T.text,fontSize:18,fontWeight:700,margin:0}}>{title}</h2>
        <button onClick={onClose} style={{background:'none',border:'none',color:T.muted,cursor:'pointer',padding:4,display:'flex'}}><X size={20}/></button>
      </div>
      {children}
    </div>
  </div>
...
const Btn=({onClick,children,variant='primary',small,disabled})=>{
  ...
  return(
    <button onClick={disabled?undefined:onClick} style={{...s[variant],padding:small?'6px 14px':'10px 20px',borderRadius:8,
      fontSize:small?12:14,fontWeight:600,cursor:disabled?'not-allowed':'pointer',display:'inline-flex',
      alignItems:'center',gap:6,opacity:disabled?0.5:1,transition:'opacity 0.15s',whiteSpace:'nowrap'}}>{children}</button>
  );
};
  ...
  return(
    <div style={{background:T.bg,borderRadius:10,padding:'12px 14px',flex:1,minWidth:110}}>
      <div style={{display:'flex',justifyContent:'space-between',marginBottom:6,alignItems:'baseline'}}>
        ...
        <span style={{color:c,fontSize:15,fontWeight:800}}>{value!=null?value:'—'}<span style={{fontSize:10,color:T.dim,fontWeight:400}}>{unit}</span></span>
      </div>
...
const LoginScreen=({onLogin})=>{
  ...
  const handle=()=>{const u=USERS0.find(u=>u.email===email&&u.pass===pass);if(u)onLogin(u);else setErr('Invalid credentials');};
  const demos=[['Super Admin','super@fleet.com','admin123'],['Org Admin','admin@mcc.com','admin123'],['Fleet Manager','manager@mcc.com','manager123'],['Driver','driver@mcc.com','driver123']];
  return(
    <div style={{minHeight:'100vh',background:T.bg,display:'flex',alignItems:'center',justifyContent:'center',fontFamily:'"Outfit",system-ui,sans-serif',padding:16,backgroundImage:`radial-gradient(ellipse at 20% 50%,rgba(245,158,11,0.06) 0%,transparent 55%),radial-gradient(ellipse at 80% 20%,rgba(96,165,250,0.06) 0%,transparent 50%)`}}>
      <div style={{width:'100%',maxWidth:420}}>
        <div style={{textAlign:'center',marginBottom:36}}>
          <div style={{display:'inline-flex',alignItems:'center',gap:12,marginBottom:10}}>
            <div style={{width:46,height:46,background:`linear-gradient(135deg,${T.accent},#d97706)`,borderRadius:13,display:'flex',alignItems:'center',justifyContent:'center',boxShadow:`0 8px 24px ${T.accent}44`}}><Truck size={24} color="#000"/></div>
        ...
        <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:16,padding:32,boxShadow:'0 20px 40px rgba(0,0,0,0.4)'}}>
          ...
          <div style={{marginBottom:14}}>
            ...
            <input type="email" value={email} onChange={e=>{setEmail(e.target.value);setErr('');}} placeholder="you@example.com" onKeyDown={e=>e.key==='Enter'&&handle()}
          ...
          <div style={{marginBottom:err?8:20,position:'relative'}}>
            ...
            <input type={show?'text':'password'} value={pass} onChange={e=>{setPass(e.target.value);setErr('');}} placeholder="••••••••" onKeyDown={e=>e.key==='Enter'&&handle()}
            ...
            <button onClick={()=>setShow(!show)} style={{position:'absolute',right:12,top:34,background:'none',border:'none',color:T.muted,cursor:'pointer',display:'flex',padding:0}}>
              {show?<EyeOff size={16}/>:<Eye size={16}/>}
            </button>
          </div>
          {err&&<p style={{color:T.red,fontSize:13,margin:'0 0 16px'}}>{err}</p>}
          <button onClick={handle} style={{width:'100%',padding:'12px',background:`linear-gradient(135deg,${T.accent},#d97706)`,border:'none',borderRadius:8,color:'#000',fontSize:15,fontWeight:700,cursor:'pointer',boxShadow:`0 4px 16px ${T.accent}44`}}>Sign In</button>
        </div>
        <div style={{marginTop:16,background:'rgba(96,165,250,0.05)',border:`1px solid ${T.blue}22`,borderRadius:12,padding:16}}>
          ...
          {demos.map(([role,e,p])=>(
            <div key={role} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'6px 0',borderBottom:`1px solid ${T.border}`}}>
              <div><span style={{color:T.text,fontSize:13,fontWeight:500}}>{role}</span><span style={{color:T.dim,fontSize:12,marginLeft:8}}>{e}</span></div>
              <button onClick={()=>{setEmail(e);setPass(p);setErr('');}} style={{background:'none',border:`1px solid ${T.border}`,color:T.muted,fontSize:11,padding:'3px 10px',borderRadius:4,cursor:'pointer'}}>Use</button>
            </div>
          ))}
        </div>
      </div>
  ...
  <div style={{width:224,background:T.sidebar,borderRight:`1px solid ${T.border}`,display:'flex',flexDirection:'column',flexShrink:0,height:'100vh',position:'sticky',top:0}}>
    <div style={{padding:'22px 20px 18px',borderBottom:`1px solid ${T.border}`}}>
      <div style={{display:'flex',alignItems:'center',gap:10}}>
        <div style={{width:34,height:34,background:`linear-gradient(135deg,${T.accent},#d97706)`,borderRadius:9,display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}><Truck size={18} color="#000"/></div>
    ...
    <nav style={{flex:1,padding:'10px 10px',overflowY:'auto'}}>
      {NAV.filter(n=>n.roles.includes(user.role)).map(({id,icon:Icon,label,live})=>{
        ...
        return(
          <button key={id} onClick={()=>setPage(id)} style={{width:'100%',display:'flex',alignItems:'center',gap:10,justifyContent:'space-between',padding:'10px 12px',borderRadius:8,border:'none',cursor:'pointer',marginBottom:2,textAlign:'left',background:active?T.accentBg:'transparent',color:active?T.accent:T.muted,transition:'all 0.12s'}}>
            <div style={{display:'flex',alignItems:'center',gap:10}}><Icon size={17}/><span style={{fontSize:14,fontWeight:active?600:400}}>{label}</span></div>
            {live&&<span style={{fontSize:9,fontWeight:700,color:T.red,background:T.redBg,padding:'2px 6px',borderRadius:4,letterSpacing:'0.05em',animation:'pulse 2s infinite'}}>LIVE</span>}
          </button>
        );
      })}
    </nav>
    <div style={{padding:12,borderTop:`1px solid ${T.border}`}}>
      <div style={{display:'flex',alignItems:'center',gap:10,padding:'10px 12px',borderRadius:8,background:T.card,marginBottom:8}}>
        <div style={{width:32,height:32,borderRadius:'50%',background:`linear-gradient(135deg,${ROLE_COLORS[user.role]},${T.accent})`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:12,fontWeight:700,color:'#fff',flexShrink:0}}>{user.initials}</div>
      ...
      <button onClick={onLogout} style={{width:'100%',display:'flex',alignItems:'center',gap:8,padding:'9px 12px',background:'none',border:`1px solid ${T.border}`,borderRadius:8,color:T.muted,cursor:'pointer',fontSize:13}}><LogOut size={15}/>Sign out</button>
    </div>
  </div>
);

// ─── DASHBOARD ────────────────────────────────────────────────────────
const Dashboard=({user,vehicles,schedules,users,orgs,simState})=>{
  ...
  const statusData=[{name:'Available',value:mv.filter(v=>v.status==='Available').length,color:T.green},{name:'In Use',value:mv.filter(v=>v.status==='In Use').length,color:T.blue},{name:'Maintenance',value:mv.filter(v=>v.status==='Maintenance').length,color:T.red},{name:'Reserved',value:mv.filter(v=>v.status==='Reserved').length,color:T.amber}].filter(d=>d.value>0);
    ...
    <div style={{padding:32}}>
      ...
      <div style={{background:T.card,border:`1px solid ${T.green}33`,borderRadius:12,padding:18,marginBottom:14}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}}>
          ...
          <span style={{color:T.muted,fontSize:11}}>Refreshes every 2s · mirrors simulator.py</span>
        </div>
        <div style={{display:'flex',gap:10,flexWrap:'wrap'}}>
          {SIM_VEHICLES.map(v=>{
            const s=simState[v.id];const hasAlert=s.activeAnomaly;
            return(
      ...
      <div style={{display:'grid',gridTemplateColumns:'1.5fr 1fr',gap:14,marginBottom:14}}>
        <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:12,padding:24}}>
          ...
          <ResponsiveContainer width="100%" height={175}>
            <AreaChart data={monthData}>
              <defs><linearGradient id="ag" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor={T.accent} stopOpacity={0.35}/><stop offset="95%" stopColor={T.accent} stopOpacity={0}/></linearGradient></defs>
        ...
        <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:12,padding:24}}>
          ...
          <div style={{display:'flex',flexDirection:'column',alignItems:'center'}}>
            <PieChart width={140} height={140}><Pie data={statusData} cx={65} cy={65} innerRadius={42} outerRadius={62} dataKey="value" paddingAngle={3}>{statusData.map((e,i)=><Cell key={i} fill={e.color}/>)}</Pie></PieChart>
            <div style={{width:'100%',marginTop:8}}>
              {statusData.map(d=>(<div key={d.name} style={{display:'flex',justifyContent:'space-between',marginBottom:6,alignItems:'center'}}>
                <div style={{display:'flex',alignItems:'center',gap:6}}><div style={{width:8,height:8,borderRadius:'50%',background:d.color}}/><span style={{color:T.muted,fontSize:12}}>{d.name}</span></div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1.3fr',gap:14}}>
        <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:12,padding:24}}>
          ...
          <ResponsiveContainer width="100%" height={150}>
            <BarChart data={catData} barSize={22}><CartesianGrid strokeDasharray="3 3" stroke={T.border} vertical={false}/><XAxis dataKey="name" tick={{fill:T.muted,fontSize:11}} axisLine={false} tickLine={false}/><YAxis tick={{fill:T.muted,fontSize:11}} axisLine={false} tickLine={false}/><Tooltip contentStyle={{background:T.card,border:`1px solid ${T.border}`,borderRadius:8,color:T.text,fontSize:13}}/><Bar dataKey="count" fill={T.blue} radius={[4,4,0,0]}/></BarChart>
          </ResponsiveContainer>
        </div>
        <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:12,padding:24}}>
          <h3 style={{color:T.text,fontSize:14,fontWeight:700,margin:'0 0 14px'}}>Recent Activity</h3>
          {ms.slice(0,4).map(s=>{const v=vehicles.find(vv=>vv.id===s.vehicleId);return(
            <div key={s.id} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'10px 12px',background:T.bg,borderRadius:8,border:`1px solid ${T.border}`,gap:10,marginBottom:8}}>
              <div style={{flex:1,overflow:'hidden'}}><p style={{color:T.text,fontSize:13,fontWeight:500,margin:'0 0 2px',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{s.purpose}</p><p style={{color:T.muted,fontSize:12,margin:0}}>{v?.plate} · {s.start}</p></div>
...
const TelemetryPage=({simState,history,apiUrl,setApiUrl,connected})=>{
  ...
  const runAlerts=async(vid)=>{
    setLoadingAlert(p=>({...p,[vid]:true}));
    if(connected){
      try{const res=await fetch(`${apiUrl}/alerts/${vid}`);const d=await res.json();setAlertData(p=>({...p,[vid]:d}));}
      catch{setAlertData(p=>({...p,[vid]:buildAlerts(history[vid]||[],vid)}));}
    }else{
  ...
  const exportCSV=vid=>{
    if(connected){window.open(`${apiUrl}/export/${vid}/csv?range=${range}`,'_blank');return;}
    ...
    const b=new Blob([lines.join('\n')],{type:'text/csv'});const a=document.createElement('a');a.href=URL.createObjectURL(b);a.download=`${vid}_telemetry.csv`;a.click();
  };
  ...
  return(
    <div style={{padding:32}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:22,flexWrap:'wrap',gap:12}}>
        <div>
          ...
          <div style={{display:'flex',alignItems:'center',gap:8}}>
            ...
            <span style={{color:T.muted,fontSize:13}}>5 vehicles streaming · 2s interval · mirrors simulator.py logic</span>
          </div>
        </div>
        <div style={{display:'flex',alignItems:'center',gap:8,background:T.card,border:`1px solid ${T.border}`,borderRadius:10,padding:'8px 14px'}}>
          ...
          <input value={apiUrl} onChange={e=>setApiUrl(e.target.value)} style={{background:'none',border:'none',color:T.text,fontSize:12,outline:'none',width:170}} placeholder="http://localhost:8000"/>
          <span style={{color:connected?T.green:T.dim,fontSize:11,fontWeight:700,whiteSpace:'nowrap'}}>{connected?'● Connected':'○ Demo Mode'}</span>
        </div>
      </div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(255px,1fr))',gap:14,marginBottom:20}}>
        {SIM_VEHICLES.map(v=>{
          const s=simState[v.id];const r=toReading(v.id,s);const isEV=v.type==='EV';const sel=selVid===v.id;const hasAlert=s.activeAnomaly;
          return(
            <div key={v.id} onClick={()=>setSelVid(sel?null:v.id)}
              style={{background:T.card,border:`2px solid ${sel?v.color:hasAlert?T.red:T.border}`,borderRadius:14,padding:20,cursor:'pointer',
                transition:'all 0.15s',boxShadow:sel?`0 0 20px ${v.color}22`:hasAlert?`0 0 12px ${T.red}22`:'none'}}>
              ...
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
                ...
                {isEV?<GaugeMini value={r.battery_pct} max={100} color={T.green} label="Battery" unit="%" warn={25} danger={15}/>
              ...
              <div style={{marginTop:12,display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                ...
                <ChevronRight size={14} color={sel?v.color:T.dim}/>
              </div>
            </div>
          );
        })}
        ...
        <div style={{background:T.card,border:`1px solid ${selVehicle.color}33`,borderRadius:14,overflow:'hidden'}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'18px 24px',borderBottom:`1px solid ${T.border}`,background:`${selVehicle.color}06`}}>
            <div style={{display:'flex',alignItems:'center',gap:12}}>
              <div style={{width:40,height:40,borderRadius:10,background:`${selVehicle.color}18`,display:'flex',alignItems:'center',justifyContent:'center'}}><Truck size={20} color={selVehicle.color}/></div>
              <div>
            ...
            <div style={{display:'flex',gap:8,alignItems:'center'}}>
              ...
              <Btn onClick={()=>{setTab('alerts');runAlerts(selVid);}} small variant="cyan"><Shield size={12}/>{loadingAlert[selVid]?'Analyzing…':'AI Alerts'}</Btn>
              <button onClick={()=>setSelVid(null)} style={{background:'none',border:'none',color:T.muted,cursor:'pointer',display:'flex',padding:4}}><X size={16}/></button>
            </div>
          </div>
          {/* Tabs */}
          <div style={{display:'flex',borderBottom:`1px solid ${T.border}`}}>
            {['charts','stats','alerts'].map(t=>(
              <button key={t} onClick={()=>{setTab(t);if(t==='alerts')runAlerts(selVid);}} style={{padding:'12px 22px',background:'none',border:'none',cursor:'pointer',
                borderBottom:`2px solid ${tab===t?selVehicle.color:'transparent'}`,color:tab===t?T.text:T.muted,
                fontSize:14,fontWeight:tab===t?600:400,textTransform:'capitalize',transition:'all 0.12s'}}>{t}</button>
            ))}
          </div>
                  ...
                  <div key={key} style={{background:T.bg,borderRadius:10,padding:16}}>
                    ...
                    <ResponsiveContainer width="100%" height={130}>
                      <AreaChart data={chartData}>
                        <defs><linearGradient id={`g-${key}`} x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor={color} stopOpacity={0.3}/><stop offset="95%" stopColor={color} stopOpacity={0}/></linearGradient></defs>
              ...
              <div>
                ...
                <div style={{background:T.bg,borderRadius:10,overflow:'auto'}}>
                  <table style={{width:'100%',borderCollapse:'collapse',minWidth:580}}>
                    <thead><tr style={{borderBottom:`1px solid ${T.border}`}}>{['Time','Speed','Temp','Battery/Fuel','RPM','Location'].map(h=><th key={h} style={{padding:'10px 14px',textAlign:'left',color:T.dim,fontSize:11,fontWeight:700,letterSpacing:'0.05em'}}>{h}</th>)}</tr></thead>
                    <tbody>
              ...
              <div>
                {!alertResult&&!loadingAlert[selVid]&&(
                  <div style={{textAlign:'center',padding:40}}>
                    <button onClick={()=>runAlerts(selVid)} style={{background:`linear-gradient(135deg,${T.cyan},${T.blue})`,border:'none',color:'#000',padding:'12px 28px',borderRadius:10,fontSize:14,fontWeight:700,cursor:'pointer',display:'inline-flex',alignItems:'center',gap:8}}><Shield size={16}/>Run AI Anomaly Analysis</button>
                    <p style={{color:T.muted,fontSize:13,marginTop:12}}>Checks last 20 readings against safety thresholds<br/>Uses same logic as <code style={{background:T.card,padding:'2px 6px',borderRadius:4,fontSize:12}}>/alerts/{'{vehicle_id}'}</code> endpoint</p>
                  </div>
                )}
                {loadingAlert[selVid]&&<div style={{textAlign:'center',padding:32,color:T.muted,fontSize:14,display:'flex',alignItems:'center',justifyContent:'center',gap:10}}><RefreshCw size={16} color={T.cyan}/>Analyzing telemetry data…</div>}
                {alertResult&&!loadingAlert[selVid]&&(
                  <div>
                    <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:20,flexWrap:'wrap',gap:10}}>
                      <div style={{display:'flex',alignItems:'center',gap:12}}>
                        <div style={{padding:'8px 20px',borderRadius:8,background:rb(alertResult.risk_level),border:`1px solid ${rl(alertResult.risk_level)}33`}}>
                          <span style={{color:rl(alertResult.risk_level),fontSize:18,fontWeight:800}}>{alertResult.risk_level==='LOW'?'✓ ':alertResult.risk_level==='HIGH'?'🔴 ':'⚠ '}{alertResult.risk_level} RISK</span>
                        </div>
                    ...
                    {alertResult.alerts.length===0&&(
                      <div style={{background:T.greenBg,border:`1px solid ${T.green}33`,borderRadius:10,padding:'16px 20px',display:'flex',alignItems:'center',gap:10,color:T.green}}><CheckCircle size={18}/><span style={{fontSize:14,fontWeight:500}}>All metrics within safe operating ranges.</span></div>
                    )}
                    {alertResult.alerts.map((a,i)=>(
                      <div key={i} style={{background:T.redBg,border:`1px solid ${T.red}33`,borderRadius:10,padding:'16px 20px',marginBottom:10}}>
                        <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:8}}><AlertTriangle size={15} color={T.red}/><span style={{color:T.red,fontSize:13,fontWeight:700,textTransform:'uppercase',letterSpacing:'0.04em'}}>{a.metric.replace('_',' ')}</span></div>
...
const VehiclesPage=({user,vehicles,setVehicles,orgs})=>{
  const [search,setSearch]=useState('');const[catF,setCatF]=useState('');const[statF,setStatF]=useState('');const[modal,setModal]=useState(null);const[form,setForm]=useState({});
  const CATS=['Sedan','SUV','Van','Truck','Bus','Motorcycle'];
  const mv=user.role==='super_admin'?vehicles:vehicles.filter(v=>v.orgId===user.orgId);
  const canEdit=['super_admin','org_admin','fleet_manager'].includes(user.role);
  const filtered=useMemo(()=>mv.filter(v=>{const s=search.toLowerCase();return(!s||v.name.toLowerCase().includes(s)||v.plate.toLowerCase().includes(s))&&(!catF||v.cat===catF)&&(!statF||v.status===statF);}),[mv,search,catF,statF]);
  ...
  const save=()=>{if(!form.plate||!form.name)return;const v={...form,year:+form.year,km:+form.km,orgId:+form.orgId};if(modal==='add')setVehicles(p=>[...p,{...v,id:Date.now()}]);else setVehicles(p=>p.map(x=>x.id===form.id?v:x));setModal(null);};
  ...
  return(
    <div style={{padding:32}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:22,flexWrap:'wrap',gap:12}}>
        <div><h1 style={{color:T.text,fontSize:24,fontWeight:800,margin:'0 0 4px',letterSpacing:'-0.5px'}}>Vehicles</h1><p style={{color:T.muted,fontSize:14,margin:0}}>{filtered.length} of {mv.length}</p></div>
        {canEdit&&<Btn onClick={openAdd}><Plus size={15}/>Add Vehicle</Btn>}
      </div>
      <div style={{display:'flex',gap:10,marginBottom:18,flexWrap:'wrap'}}>
        <div style={{flex:1,minWidth:200,position:'relative'}}><Search size={14} style={{position:'absolute',left:11,top:'50%',transform:'translateY(-50%)',color:T.muted}}/><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search plate or name…" style={{width:'100%',padding:'9px 9px 9px 34px',background:T.card,border:`1px solid ${T.border}`,borderRadius:8,color:T.text,fontSize:13,outline:'none',boxSizing:'border-box'}}/></div>
        <select value={catF} onChange={e=>setCatF(e.target.value)} style={{padding:'9px 12px',background:T.card,border:`1px solid ${T.border}`,borderRadius:8,color:catF?T.text:T.muted,fontSize:13,outline:'none',cursor:'pointer'}}><option value="">All Categories</option>{CATS.map(c=><option key={c} value={c}>{c}</option>)}</select>
        <select value={statF} onChange={e=>setStatF(e.target.value)} style={{padding:'9px 12px',background:T.card,border:`1px solid ${T.border}`,borderRadius:8,color:statF?T.text:T.muted,fontSize:13,outline:'none',cursor:'pointer'}}><option value="">All Statuses</option>{['Available','In Use','Maintenance','Reserved'].map(s=><option key={s} value={s}>{s}</option>)}</select>
      </div>
      <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:12,overflow:'auto'}}>
        <table style={{width:'100%',borderCollapse:'collapse',minWidth:700}}>
          <thead style={{borderBottom:`1px solid ${T.border}`}}><tr>{['PLATE','VEHICLE','CAT','ORG','STATUS','YEAR','KM',''].map(c=><TH key={c} c={c}/>)}</tr></thead>
          <tbody>
            {filtered.map((v,i)=>{const org=orgs.find(o=>o.id===v.orgId);return(
              <tr key={v.id} style={{borderBottom:i<filtered.length-1?`1px solid ${T.border}`:'none'}} onMouseEnter={e=>e.currentTarget.style.background=T.cardHover} onMouseLeave={e=>e.currentTarget.style.background='transparent'}>
                ...
                <td style={{padding:'13px 16px'}}>{canEdit&&<div style={{display:'flex',gap:6}}><button onClick={()=>openEdit(v)} style={{background:T.blueBg,border:'none',color:T.blue,padding:'5px 10px',borderRadius:6,cursor:'pointer',fontSize:12,display:'flex',alignItems:'center',gap:4}}><Edit2 size={12}/>Edit</button><button onClick={()=>del(v.id)} style={{background:T.redBg,border:'none',color:T.red,padding:'5px 8px',borderRadius:6,cursor:'pointer',display:'flex',alignItems:'center'}}><Trash2 size={13}/></button></div>}</td>
              </tr>
            );})}
            {filtered.length===0&&<tr><td colSpan={8} style={{padding:48,textAlign:'center',color:T.muted,fontSize:14}}>No vehicles found</td></tr>}
          </tbody>
        </table>
      </div>
      {modal&&<Modal title={modal==='add'?'Add Vehicle':'Edit Vehicle'} onClose={()=>setModal(null)}>
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}><Field label="Plate" value={form.plate} onChange={v=>setForm(f=>({...f,plate:v}))} placeholder="MCC-007"/><Field label="Vehicle Name" value={form.name} onChange={v=>setForm(f=>({...f,name:v}))} placeholder="Toyota Camry"/></div>
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}><Field label="Category" value={form.cat} onChange={v=>setForm(f=>({...f,cat:v}))} options={CATS}/><Field label="Status" value={form.status} onChange={v=>setForm(f=>({...f,status:v}))} options={['Available','In Use','Maintenance','Reserved']}/></div>
        {user.role==='super_admin'&&<Field label="Organization" value={form.orgId} onChange={v=>setForm(f=>({...f,orgId:v}))} options={orgs.map(o=>({value:o.id,label:o.name}))}/>}
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}><Field label="Year" value={form.year} onChange={v=>setForm(f=>({...f,year:v}))} type="number"/><Field label="Mileage (km)" value={form.km} onChange={v=>setForm(f=>({...f,km:v}))} type="number"/></div>
        <div style={{display:'flex',gap:10,justifyContent:'flex-end',marginTop:8}}><Btn onClick={()=>setModal(null)} variant="secondary">Cancel</Btn><Btn onClick={save} disabled={!form.plate||!form.name}><CheckCircle size={14}/>Save</Btn></div>
      </Modal>}
    </div>
  );
};
...
const SchedulesPage=({user,schedules,setSchedules,vehicles,users})=>{
  const [modal,setModal]=useState(false);const[statF,setStatF]=useState('');
  const [form,setForm]=useState({vehicleId:'',userId:'',purpose:'',start:fd(nowD),end:fd(nowD)});const[conflict,setConflict]=useState(false);
  const ms=user.role==='super_admin'?schedules:user.role==='driver'?schedules.filter(s=>s.userId===user.id):schedules.filter(s=>s.orgId===user.orgId);
  const mv=user.role==='super_admin'?vehicles:vehicles.filter(v=>v.orgId===user.orgId);
  ...
  const checkConflict=(vid,start,end)=>schedules.some(s=>s.vehicleId===+vid&&s.status!=='cancelled'&&s.status!=='completed'&&start<=s.end&&end>=s.start);
  const upd=(field,val)=>{const nf={...form,[field]:val};setForm(nf);if(nf.vehicleId&&nf.start&&nf.end)setConflict(checkConflict(nf.vehicleId,nf.start,nf.end));};
  const save=()=>{if(!form.vehicleId||!form.purpose||conflict)return;const orgId=user.role==='super_admin'?vehicles.find(v=>v.id===+form.vehicleId)?.orgId:user.orgId;setSchedules(p=>[...p,{id:Date.now(),vehicleId:+form.vehicleId,userId:form.userId?+form.userId:user.id,orgId,purpose:form.purpose,start:form.start,end:form.end,status:'reserved'}]);setModal(false);setConflict(false);};
  const updStatus=(id,status)=>setSchedules(p=>p.map(s=>s.id===id?{...s,status}:s));
  return(
    <div style={{padding:32}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:22,flexWrap:'wrap',gap:12}}>
        <div><h1 style={{color:T.text,fontSize:24,fontWeight:800,margin:'0 0 4px',letterSpacing:'-0.5px'}}>Schedules</h1><p style={{color:T.muted,fontSize:14,margin:0}}>{filtered.length} bookings</p></div>
        {canAdd&&<Btn onClick={()=>setModal(true)}><Plus size={15}/>New Booking</Btn>}
      </div>
      <div style={{display:'flex',gap:6,marginBottom:20,flexWrap:'wrap'}}>
        {['',...['active','reserved','completed','cancelled']].map(s=>(
          <button key={s} onClick={()=>setStatF(s)} style={{padding:'7px 16px',borderRadius:20,cursor:'pointer',fontSize:13,border:`1px solid ${statF===s?(sc(s)||T.accent):T.border}`,fontWeight:statF===s?600:400,background:statF===s?(sb(s)||T.accentBg):'transparent',color:statF===s?(sc(s)||T.accent):T.muted,textTransform:'capitalize'}}>
            {s||'All'} {s&&`(${ms.filter(x=>x.status===s).length})`}
          </button>
        ))}
      </div>
      <div style={{display:'flex',flexDirection:'column',gap:10}}>
        {filtered.sort((a,b)=>b.start.localeCompare(a.start)).map(s=>{const v=vehicles.find(vv=>vv.id===s.vehicleId);const u=users.find(uu=>uu.id===s.userId);return(
          <div key={s.id} style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:12,padding:20,display:'flex',justifyContent:'space-between',alignItems:'center',gap:16,flexWrap:'wrap'}}>
            <div style={{flex:1,minWidth:200}}>
              <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:6}}><Badge label={s.status} color={sc(s.status)} bg={sb(s.status)}/><span style={{color:T.dim,fontSize:11}}>#{s.id}</span></div>
            ...
            {user.role!=='driver'&&<div style={{display:'flex',gap:8,flexShrink:0}}>
              {s.status==='reserved'&&<><Btn onClick={()=>updStatus(s.id,'active')} small variant="secondary"><Activity size={12}/>Activate</Btn><Btn onClick={()=>updStatus(s.id,'cancelled')} small variant="danger"><X size={12}/>Cancel</Btn></>}
              {s.status==='active'&&<Btn onClick={()=>updStatus(s.id,'completed')} small><CheckCircle size={12}/>Complete</Btn>}
            </div>}
          </div>
        );})}
        {filtered.length===0&&<div style={{padding:56,textAlign:'center',color:T.muted,background:T.card,borderRadius:12,border:`1px solid ${T.border}`,fontSize:14}}>No schedules found</div>}
      </div>
      {modal&&<Modal title="Create Booking" onClose={()=>{setModal(false);setConflict(false);}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}><Field label="Start" value={form.start} onChange={v=>upd('start',v)} type="date"/><Field label="End" value={form.end} onChange={v=>upd('end',v)} type="date"/></div>
        {conflict&&<div style={{background:T.redBg,border:`1px solid ${T.red}33`,borderRadius:8,padding:'10px 14px',marginBottom:12,color:T.red,fontSize:13,display:'flex',alignItems:'center',gap:8}}><AlertCircle size={15}/>Conflict: vehicle already booked for these dates.</div>}
        <div style={{display:'flex',gap:10,justifyContent:'flex-end'}}><Btn onClick={()=>setModal(false)} variant="secondary">Cancel</Btn><Btn onClick={save} disabled={!form.vehicleId||!form.purpose||conflict}><CheckCircle size={14}/>Confirm</Btn></div>
      </Modal>}
    </div>
  );
};
...
const OrgsPage=({orgs,setOrgs,vehicles,users})=>{
  const [modal,setModal]=useState(null);const[form,setForm]=useState({name:'',code:'',status:'active'});
  const save=()=>{if(!form.name||!form.code)return;if(modal==='add')setOrgs(p=>[...p,{...form,id:Date.now(),createdAt:fd(nowD)}]);else setOrgs(p=>p.map(o=>o.id===form.id?form:o));setModal(null);};
  return(
    <div style={{padding:32}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:24,flexWrap:'wrap',gap:12}}>
        <div><h1 style={{color:T.text,fontSize:24,fontWeight:800,margin:'0 0 4px',letterSpacing:'-0.5px'}}>Organizations</h1><p style={{color:T.muted,fontSize:14,margin:0}}>{orgs.length} organizations</p></div>
        <Btn onClick={()=>{setForm({name:'',code:'',status:'active'});setModal('add');}}><Plus size={15}/>Add</Btn>
      </div>
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(280px,1fr))',gap:16}}>
        {orgs.map(o=>{const vc2=vehicles.filter(v=>v.orgId===o.id).length;const uc=users.filter(u=>u.orgId===o.id).length;const avail=vehicles.filter(v=>v.orgId===o.id&&v.status==='Available').length;return(
          <div key={o.id} style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:12,padding:24}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:8,marginBottom:18,background:T.bg,borderRadius:8,padding:12}}>
              {[['Vehicles',vc2,T.blue],['Available',avail,T.green],['Users',uc,T.amber]].map(([l,v,c])=>(
                <div key={l} style={{textAlign:'center'}}><p style={{color:c,fontSize:20,fontWeight:800,margin:'0 0 2px'}}>{v}</p><p style={{color:T.dim,fontSize:11,margin:0}}>{l}</p></div>
              ))}
            </div>
            <div style={{display:'flex',gap:8}}>
              <button onClick={()=>{setForm({...o});setModal('edit');}} style={{flex:1,padding:'8px',background:T.blueBg,border:'none',color:T.blue,borderRadius:7,cursor:'pointer',fontSize:13,display:'flex',alignItems:'center',justifyContent:'center',gap:5}}><Edit2 size={13}/>Edit</button>
              <button onClick={()=>setOrgs(p=>p.filter(x=>x.id!==o.id))} style={{padding:'8px 12px',background:T.redBg,border:'none',color:T.red,borderRadius:7,cursor:'pointer',display:'flex',alignItems:'center'}}><Trash2 size={14}/></button>
            </div>
          </div>
        );})}
      </div>
      {modal&&<Modal title={modal==='add'?'Add Organization':'Edit Organization'} onClose={()=>setModal(null)}>
        ...
        <div style={{display:'flex',gap:10,justifyContent:'flex-end'}}><Btn onClick={()=>setModal(null)} variant="secondary">Cancel</Btn><Btn onClick={save} disabled={!form.name||!form.code}><CheckCircle size={14}/>Save</Btn></div>
      </Modal>}
    </div>
  );
};
...
const UsersPage=({user,users,setUsers,orgs})=>{
  ...
  const save=()=>{if(!form.name||!form.email)return;const initials=form.name.split(' ').map(n=>n[0]).join('').toUpperCase().slice(0,2);if(modal==='add')setUsers(p=>[...p,{...form,id:Date.now(),initials}]);else setUsers(p=>p.map(u=>u.id===form.id?{...form,initials}:u));setModal(null);};
  const TH=({c})=><th style={{padding:'12px 16px',textAlign:'left',color:T.dim,fontSize:11,fontWeight:700,letterSpacing:'0.06em'}}>{c}</th>;
  return(
    <div style={{padding:32}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:22,flexWrap:'wrap',gap:12}}>
        <div><h1 style={{color:T.text,fontSize:24,fontWeight:800,margin:'0 0 4px',letterSpacing:'-0.5px'}}>Users</h1><p style={{color:T.muted,fontSize:14,margin:0}}>{filtered.length} users</p></div>
        <Btn onClick={()=>{setForm({name:'',email:'',pass:'password',role:'driver',orgId:user.orgId||1});setModal('add');}}><Plus size={15}/>Add User</Btn>
      </div>
      <div style={{position:'relative',marginBottom:18,maxWidth:320}}><Search size={14} style={{position:'absolute',left:11,top:'50%',transform:'translateY(-50%)',color:T.muted}}/><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search…" style={{width:'100%',padding:'9px 9px 9px 34px',background:T.card,border:`1px solid ${T.border}`,borderRadius:8,color:T.text,fontSize:13,outline:'none',boxSizing:'border-box'}}/></div>
      <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:12,overflow:'auto'}}>
        <table style={{width:'100%',borderCollapse:'collapse',minWidth:600}}>
          <thead style={{borderBottom:`1px solid ${T.border}`}}><tr>{['USER','EMAIL','ROLE','ORGANIZATION',''].map(c=><TH key={c} c={c}/>)}</tr></thead>
          <tbody>
            {filtered.map((u,i)=>{const org=orgs.find(o=>o.id===u.orgId);const rc=ROLE_COLORS[u.role]||T.muted;return(
              <tr key={u.id} style={{borderBottom:i<filtered.length-1?`1px solid ${T.border}`:'none'}} onMouseEnter={e=>e.currentTarget.style.background=T.cardHover} onMouseLeave={e=>e.currentTarget.style.background='transparent'}>
                <td style={{padding:'13px 16px'}}><div style={{display:'flex',alignItems:'center',gap:10}}><div style={{width:34,height:34,borderRadius:'50%',background:`${rc}18`,border:`2px solid ${rc}33`,display:'flex',alignItems:'center',justifyContent:'center',color:rc,fontSize:12,fontWeight:700}}>{u.initials}</div><div><p style={{color:T.text,fontSize:14,fontWeight:600,margin:0}}>{u.name}</p>{u.id===user.id&&<span style={{color:T.accent,fontSize:11}}>● You</span>}</div></div></td>
                ...
                <td style={{padding:'13px 16px'}}><div style={{display:'flex',gap:6}}><button onClick={()=>{setForm({...u});setModal('edit');}} style={{background:T.blueBg,border:'none',color:T.blue,padding:'5px 10px',borderRadius:6,cursor:'pointer',fontSize:12,display:'flex',alignItems:'center',gap:4}}><Edit2 size={12}/>Edit</button>{u.id!==user.id&&<button onClick={()=>setUsers(p=>p.filter(x=>x.id!==u.id))} style={{background:T.redBg,border:'none',color:T.red,padding:'5px 8px',borderRadius:6,cursor:'pointer',display:'flex',alignItems:'center'}}><Trash2 size={13}/></button>}</div></td>
              </tr>
            );})}
          </tbody>
        </table>
      ...
      {modal&&<Modal title={modal==='add'?'Add User':'Edit User'} onClose={()=>setModal(null)}>
        ...
        <Field label="Role" value={form.role} onChange={v=>setForm(f=>({...f,role:v}))} options={Object.entries(ROLES_MAP).filter(([k])=>user.role==='super_admin'||k!=='super_admin').map(([value,label])=>({value,label}))}/>
        {user.role==='super_admin'&&<Field label="Organization" value={form.orgId||''} onChange={v=>setForm(f=>({...f,orgId:+v||null}))} options={[{value:'',label:'None (Platform Admin)'},...orgs.map(o=>({value:o.id,label:o.name}))]}/>}
        <div style={{display:'flex',gap:10,justifyContent:'flex-end'}}><Btn onClick={()=>setModal(null)} variant="secondary">Cancel</Btn><Btn onClick={save} disabled={!form.name||!form.email}><CheckCircle size={14}/>Save</Btn></div>
      </Modal>}
    </div>
  );
};
...
const ReportsPage=({user,vehicles,schedules,orgs,history})=>{
  ...
  const CATS=['Sedan','SUV','Van','Truck','Bus'];
  const utilData=CATS.map(cat=>{const total=mv.filter(v=>v.cat===cat).length;const used=mv.filter(v=>v.cat===cat&&(v.status==='In Use'||v.status==='Reserved')).length;return{cat,total,pct:total?Math.round(used/total*100):0};}).filter(d=>d.total>0);
  const statusDist=[{s:'Available',count:mv.filter(v=>v.status==='Available').length,color:T.green},{s:'In Use',count:mv.filter(v=>v.status==='In Use').length,color:T.blue},{s:'Maintenance',count:mv.filter(v=>v.status==='Maintenance').length,color:T.red},{s:'Reserved',count:mv.filter(v=>v.status==='Reserved').length,color:T.amber}];
  const orgData=user.role==='super_admin'?orgs.map(o=>({org:o.code,vehicles:vehicles.filter(v=>v.orgId===o.id).length,active:schedules.filter(s=>s.orgId===o.id&&(s.status==='active'||s.status==='reserved')).length,completed:schedules.filter(s=>s.orgId===o.id&&s.status==='completed').length})):[];
  const liveSpeedData=SIM_VEHICLES.map(v=>{const h=history[v.id]||[];const avg=h.length?+(h.slice(0,10).reduce((s,r)=>s+(r.speed||0),0)/Math.min(10,h.length)).toFixed(1):0;return{name:v.id,avg,color:v.color};});
  const utilPct=mv.length?Math.round(mv.filter(v=>v.status==='In Use'||v.status==='Reserved').length/mv.length*100):0;
  return(
    <div style={{padding:32}}>
      <div style={{marginBottom:26}}><h1 style={{color:T.text,fontSize:24,fontWeight:800,margin:'0 0 4px',letterSpacing:'-0.5px'}}>Reports</h1><p style={{color:T.muted,fontSize:14,margin:0}}>Fleet performance & telemetry analytics</p></div>
      <div style={{display:'flex',gap:14,marginBottom:22,flexWrap:'wrap'}}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14,marginBottom:14}}>
        <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:12,padding:24}}>
          ...
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={utilData}><CartesianGrid strokeDasharray="3 3" stroke={T.border} vertical={false}/><XAxis dataKey="cat" tick={{fill:T.muted,fontSize:12}} axisLine={false} tickLine={false}/><YAxis tick={{fill:T.muted,fontSize:12}} axisLine={false} tickLine={false} domain={[0,100]} unit="%"/><Tooltip contentStyle={{background:T.card,border:`1px solid ${T.border}`,borderRadius:8,color:T.text,fontSize:13}} formatter={v=>[`${v}%`,'Utilization']}/><Bar dataKey="pct" fill={T.accent} radius={[4,4,0,0]} barSize={26}/></BarChart>
          </ResponsiveContainer>
        </div>
        <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:12,padding:24}}>
          ...
          {statusDist.map(d=>(
            <div key={d.s} style={{marginBottom:14}}>
              <div style={{display:'flex',justifyContent:'space-between',marginBottom:6}}><span style={{color:T.muted,fontSize:13}}>{d.s}</span><span style={{color:T.text,fontSize:13,fontWeight:700}}>{d.count}</span></div>
              <div style={{height:7,background:T.bg,borderRadius:4,overflow:'hidden'}}><div style={{height:'100%',background:d.color,borderRadius:4,width:`${mv.length?d.count/mv.length*100:0}%`,transition:'width 0.5s'}}/></div>
            </div>
          ))}
        </div>
      </div>
      <div style={{background:T.card,border:`1px solid ${T.cyan}33`,borderRadius:12,padding:24,marginBottom:14}}>
        <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:16}}><span style={{width:8,height:8,borderRadius:'50%',background:T.cyan,display:'inline-block',animation:'pulse 2s infinite'}}/><h3 style={{color:T.text,fontSize:14,fontWeight:700,margin:0}}>Live Avg Speed — Telemetry Vehicles</h3></div>
        <ResponsiveContainer width="100%" height={160}>
          <BarChart data={liveSpeedData}><CartesianGrid strokeDasharray="3 3" stroke={T.border} vertical={false}/><XAxis dataKey="name" tick={{fill:T.muted,fontSize:12}} axisLine={false} tickLine={false}/><YAxis tick={{fill:T.muted,fontSize:12}} axisLine={false} tickLine={false} unit=" km/h"/><Tooltip contentStyle={{background:T.card,border:`1px solid ${T.border}`,borderRadius:8,color:T.text,fontSize:13}} formatter={v=>[`${v} km/h`,'Avg Speed']}/><Bar dataKey="avg" radius={[5,5,0,0]} barSize={36}>{liveSpeedData.map((e,i)=><Cell key={i} fill={e.color}/>)}</Bar></BarChart>
        </ResponsiveContainer>
      </div>
      {user.role==='super_admin'&&(
        <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:12,padding:24}}>
          ...
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={orgData} barGap={4}><CartesianGrid strokeDasharray="3 3" stroke={T.border} vertical={false}/><XAxis dataKey="org" tick={{fill:T.muted,fontSize:13}} axisLine={false} tickLine={false}/><YAxis tick={{fill:T.muted,fontSize:12}} axisLine={false} tickLine={false}/><Tooltip contentStyle={{background:T.card,border:`1px solid ${T.border}`,borderRadius:8,color:T.text,fontSize:13}}/><Legend wrapperStyle={{color:T.muted,fontSize:12}}/><Bar dataKey="vehicles" fill={T.blue} radius={[4,4,0,0]} barSize={22} name="Vehicles"/><Bar dataKey="active" fill={T.accent} radius={[4,4,0,0]} barSize={22} name="Active"/><Bar dataKey="completed" fill={T.green} radius={[4,4,0,0]} barSize={22} name="Completed"/></BarChart>
          </ResponsiveContainer>
        </div>
      )}
    </div>
...
const TopBar=({page,user,alerts})=>{
  const titles={dashboard:'Dashboard',telemetry:'Live Telemetry',vehicles:'Vehicles',schedules:'Schedules',organizations:'Organizations',users:'Users',reports:'Reports'};
  return(
    <div style={{height:60,borderBottom:`1px solid ${T.border}`,display:'flex',alignItems:'center',justifyContent:'space-between',padding:'0 32px',background:T.bg,position:'sticky',top:0,zIndex:10}}>
      <div style={{display:'flex',alignItems:'center',gap:8}}><span style={{color:T.dim,fontSize:13}}>FleetOS</span><span style={{color:T.border}}>›</span><span style={{color:T.text,fontSize:13,fontWeight:600}}>{titles[page]}</span></div>
      <div style={{display:'flex',alignItems:'center',gap:10}}>
        ...
        {alerts>0&&<div style={{background:T.redBg,color:T.red,padding:'3px 10px',borderRadius:20,fontSize:12,fontWeight:600,border:`1px solid ${T.red}33`,animation:'pulse 2s infinite'}}><AlertTriangle size={11} style={{marginRight:4,display:'inline'}}/>{alerts} Alert{alerts>1?'s':''}</div>}
...
export default function FleetApp(){
  useEffect(()=>{
    const l=document.createElement('link');l.href='https://fonts.googleapis.com/css2?family=Outfit:wght@400;500;600;700;800&display=swap';l.rel='stylesheet';document.head.appendChild(l);
  },[]);
  ...
  // Telemetry state
  const [simState,setSimState]=useState(()=>initSimState());
  const [history,setHistory]=useState(()=>{const h={};SIM_VEHICLES.forEach(v=>{h[v.id]=[];});return h;});
  const [apiUrl,setApiUrl]=useState('http://localhost:8000');
  const [connected,setConnected]=useState(false);

  // Ping backend every 10s
  useEffect(()=>{
    const ping=async()=>{try{const r=await fetch(`${apiUrl}/`,{signal:AbortSignal.timeout(2000)});setConnected(r.ok);}catch{setConnected(false);}};
    ping();const t=setInterval(ping,10000);return()=>clearInterval(t);
  },[apiUrl]);

  // Simulator tick — 2s, identical to simulator.py
  useEffect(()=>{
    const t=setInterval(()=>{
      const ts=new Date().toISOString();
      setSimState(prev=>{
        const next=tickSimState(prev);
        setHistory(h=>{
          const nh={...h};
          SIM_VEHICLES.forEach(v=>{nh[v.id]=[toReading(v.id,next[v.id],ts),...(h[v.id]||[])].slice(0,200);});
          return nh;
        });
        return next;
      });
  ...
  const PAGES={
    dashboard:<Dashboard {...p} simState={simState}/>,
    telemetry:<TelemetryPage simState={simState} history={history} apiUrl={apiUrl} setApiUrl={setApiUrl} connected={connected}/>,
    vehicles:<VehiclesPage {...p}/>,
    schedules:<SchedulesPage {...p}/>,
    organizations:<OrgsPage {...p}/>,
    users:<UsersPage {...p}/>,
    reports:<ReportsPage {...p} history={history}/>,
  };
  return(
